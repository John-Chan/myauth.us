...
var Creation = Class.extend({
	...
	setup: function() {
		...
		if (this.countrySelect.length) {
			...
			// If no country is selected by default, disable the rest of the form.
			if (!this.countryDefault) {
				this.countryBlockable.find('select, input').attr('disabled', 'disabled');
		...
		// Bind the events for warning toggle
		if (this.focusElements.length) {
			for (var i = 0, len = this.focusElements.length; i < len; i++) {
				var field = $(this.focusElements[i]);
				if (field.is('select')) {
					field.bind('change', { form: this }, function(e) {
						e.stopPropagation();
						e.data.form.toggleWarnings(this);
					});
				}

				field.bind('focus click', { form: this }, function(e) {
					if (!$(this).is('a')) {
						e.stopPropagation();
						e.data.form.toggleWarnings(this);
					}
		...
		// Bind the form validate event.
		this.form.bind('validate', { form: this }, function(e) {
			if ($('#emailAddress').val() && document.activeElement.id != 'emailAddress' && document.activeElement.id != 'emailAddressConfirmation') {
				e.data.form.validateEmail();
			}
			if ($('#parentContactEmail').val() && document.activeElement.id != 'parentContactEmail' && document.activeElement.id != 'parentContactEmailConfirmation') {
				e.data.form.showContactInfo = false;
		...
		this.form.bind('submit', { form: this }, function(e) {
			var blank = false,
				...
				errorMessages = [],
				field = '',
				fields = $('#creation [required]');

			if (e.data.form.countryBlocked) {
				errorMessages.push(FormMsg.selectCountry);
				e.data.form.toggleErrorFlag(e.data.form.countrySelect, true);
			} else {
				for (var i = 0, len = fields.length; i < len; i++) {
					field = fields[i];
					if (!field.value) {
						if (!blank) {
							errorMessages.push(FormMsg.fieldInvalid, FormMsg.fieldsMissing);
							blank = true;
						}
						e.data.form.toggleErrorFlag(field, true);
					} else if (field.id.indexOf('email') > -1 && !e.data.form.emailValidated) {
						if (!badEmail) {
							errorMessages.push(FormMsg.emailError1);
							badEmail = true;
						}
						e.data.form.toggleErrorFlag(field, true);
					} else if (field.id.indexOf('password') > -1 && !e.data.form.passwordValidated) {
						errorMessages.push(FormMsg.passwordError1);
						e.data.form.toggleErrorFlag(field, true);
					} else if (field.type == 'checkbox' && !field.checked) {
						if (!unchecked) {
							errorMessages.push(FormMsg.touDisagree);
							unchecked = true;
						}
						e.data.form.toggleErrorFlag(field, true);
					} else {
						e.data.form.toggleErrorFlag(field, false);
					}
				}
			}
			
			if (errorMessages.length) {
				$('#content').prepend(e.data.form.makeErrorBox(errorMessages));
				$('#password-strength').slideUp(e.data.form.slideUpDuration);
		...
		if (this.config.showContactInfo) {
			...
			this.contactInput1.bind('focus input propertychange',
				$.proxy(this._validateContact, this)
			);

			this.contactInput2.bind('focus input propertychange',
				$.proxy(this._validateContact, this)
			);
		...
		this.battleTagInput1.bind('focus input',
			$.proxy(this._togglebattleTagNote, this)
		);

		// Dynamic password strength rating.
		this.passwordInput1.bind('focus input',
			$.proxy(this._validatePassword, this)
		);

		this.passwordInput2.bind('focus input',
			$.proxy(this._validatePassword, this)
		);

		// Secret question information
		this.secretQuestion.bind('focus',
			$.proxy(this._toggleSecretNote, this)
		);
		this.secretAnswer.bind('focus',
			$.proxy(this._toggleSecretNote, this)
		);
	...
	_validateEmail: function(e) {
		...
		} else if (target === email2 && length1 > length2) {
	...
	validateContact: function() {
		...
		if (this.showContactInfo) {
			...
			this.contactInfo.slideDown(this.slideDownDuration);
			...
			var form = this;
			setTimeout(function() {
				form.contactInfo.slideUp(this.slideUpDuration);
			}, 100);
		}
	...
	_togglebattleTagNote: function(e) {
		...
		this.showbattleTagGuidelines = true;
	...
	/**
	 * Toggle inline validation warnings
	 */
	toggleWarnings: function(elem) {
		var activeId = elem.id,
			bind = this;
		
		for (var i = 0, len = this.warningLines.length; i < len; i++) {
			var inline = $(this.warningLines[i]),
				inlineId = inline.attr('id').replace(/-message/,'');
			if (inlineId != activeId && !inline.hasClass('no-text-clear') && !inline.children('.inline-error').length) {
				inline.html(' ');
			} else if (inlineId == 'question1' || inlineId == 'answer1') {
				$('#question-info').css('display', (activeId == 'question1' || activeId == 'answer1') ? 'block' : 'none');
			}
		}

		if (activeId == 'password' || activeId == 'rePassword') {
			this.passwordGuidelines.slideDown(this.slideDownDuration);
			if (this.config.showContactInfo) {
				this.contactInfo.slideUp(this.slideUpDuration);
			}
		} else if (activeId == 'parentContactEmail' || activeId == 'parentContactEmailConfirmation') {
			this.passwordGuidelines.slideUp(this.slideUpDuration);
			if (this.config.showContactInfo) {
				this.contactInfo.slideDown(this.slideDownDuration);
			}
		} else if (activeId != 'creation-submit' && elem.tagName != 'a') {
			setTimeout(function() {
				bind.passwordGuidelines.slideUp(bind.slideUpDuration);
			}, 100);
			if (this.config.showContactInfo) {
				this.contactInfo.slideUp(this.slideUpDuration);
			}
	...
	/**
	 * Toggle error highlighting on form fields and labels
	 */
	toggleErrorFlag: function(field, hasError) {
		field = $(field);
		if (hasError) {
			field.parents('span.input-right').addClass('input-error');
			field.parents('span.input-right').siblings('span.input-left').addClass('input-error');
		} else {
			field.parents('span.input-right').removeClass('input-error');
			field.parents('span.input-right').siblings('span.input-left').removeClass('input-error');
		}
	},
	...
	/**
	 * Create container to display submission errors.
	 *
	 * @param errorMsgs An array containing the error messages to display.
	 */
...
var Confirmation = Class.extend({
	...
	setup: function() {
		...
		this.form.bind('submit', { form: this }, function(e) {
			...
			if ($('#parentEmail').val() && e.data.form.contactValidated) {
				validEntry = true;
			}
			return validEntry;
		});
