...
var Lobby = {
	...
	createDashboardLink: function(target) {
        var trackingUrl = $(target).find('a').attr('rel');
        if(trackingUrl) {
    ...
    getSecurityDetails: function(div) {
         $.ajax({
			timeout: 60000,
            url: Core.baseUrl + '/data/phone-secure-status.html?_=' + new Date().getTime(),
			beforeSend: function() {
    ...
    getPaymentDetails: function(div) {
        $.ajax({
			timeout: 60000,
            url: Core.baseUrl + '/data/payment-details.html?_=' + new Date().getTime(),
			beforeSend: function() {
				$(div).removeClass('unloaded').addClass('loading');
			},
            success: function(msg) {
