...
var CharSelect = {
	...
	pin: function(index, node) {
		...
		$.ajax({
			type: 'POST',
			url: Core.baseUrl + '/pref/character',
			data: {
				index: index,
			...
			success: function(response) {
				...
				if (Core.isCallback(CharSelect.beforeCallback) && CharSelect.beforeCallback(node)) {
					return;
				}
 ...
 * Lazy load will reveal hidden elements in the page once they are within the viewport, after a scroll or resize.
 */
var LazyLoad = {

	/**
	 * How long to wait after DOMReady to pre-fetch the rest of the images.
	 */
	delay: 7500,

	/**
	 * Threshold gap when determining viewport range.
	 */
	threshold: 150,

	/**
	 * Has all the elements been force loaded?
	 */
	loaded: false,

	/**
	 * Initialize the handler to the scroll and resize events.
	 */
	initialize: function() {
		var style = '<style type="text/css">' +
			'.lazy-load { background: none !important; }' +
		...
		// Bind events
		$(window).bind('scroll.lazyload resize.lazyload', LazyLoad.reveal);

		// Reveal elements already in the viewport
		$(function() {
			LazyLoad.reveal();

			if (LazyLoad.delay > 0) {
				window.setTimeout(LazyLoad.loadAll, LazyLoad.delay);
			}
		});
	},

	/**
	 * Reveal the elements on the page by removing the .lazy-load class.
	 */
	reveal: function() {
		if (LazyLoad.loaded) {
			return false;
	...
	/**
	 * Validate the element is within the viewable area.
	 *
	 * @param node
	 * @return boolean
	 */
	inViewport: function(node) {
		...
		return (
			...
			(nodeOffset.top <= (scrollTop + pageHeight + threshold)) &&
			// Right of the left fold
			(nodeOffset.left >= (scrollLeft - threshold)) &&
			// Left of the right fold
			(nodeOffset.left <= (scrollLeft + pageWidth + threshold))
		);
	}
...
var Prompt = {
	...
	initialize: function() {
		...
		$('<button type="button"/>')
			...
			.html('<span class="button-left"><span class="button-right">' + Msg.ui.submit + '</span></span>')
			.click(Prompt.validate)
			.appendTo(buttons);

		$('<button type="button"/>')
			.addClass('ui-button button1')
			.html('<span class="button-left"><span class="button-right">' + Msg.ui.cancel + '</span></span>')
			.click(Prompt.close)
			.appendTo(buttons);
...
var Search = {
	...
	map: {
		...
		blog: 'article',
		product: 'product',
		'static': 'static', /* quote property name to avoid reserved word issue with compressor */
		friend: 'friend'
	},
	...
	initialize: function(url, searchField) {
		...
		if (Core.project === 'wow') {
			...
			resultTypes = ['friend', 'url', 'product', 'article', 'static', 'other', 'kb'];
		}

		Search.ta = TypeAhead.factory(searchField || '#search-field', {
			groupResults: true,
			resultTypes: resultTypes,
			ghostQuery: (Core.region === 'us' || Core.region === 'eu' || Core.region === 'sea'),
...
var Tooltip = {
	...
	init: function(query, options) {
		...
		if (!options.useTable) {
			this.contentCell = $('<div/>').addClass('tooltip-content').appendTo(wrapper);
		...
		var event = (options.mode === 'click') ? 'click' : 'mouseover',
			callback = (typeof options.onShow === 'function') ? options.onShow : function(e) {
		...
		$(options.context || document)
			.off(event + '.tooltip', query, $.proxy(callback, this))
			.on(event + '.tooltip', query, $.proxy(callback, this));
	...
	show: function(node, content, options) {
		...
		if (options.location === this.options.location && node.offset().left > $(window).width() / 2) {
	...
	_mouse: function(width, height, node) {
		node.unbind('mousemove.tooltip').bind('mousemove.tooltip', $.proxy(function(e) {
